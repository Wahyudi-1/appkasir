...
// GANTI "URL_ANDA_DI_SINI" DENGAN URL WEB APP ANDA
const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbxcMdtPnWusiUybkZScG26mjtN6UTJzEUsOm8ZlPj1B5mM-h_iTWVXjRngLjaxLvms34Q/exec"; 
...
function renderTabelBarang() { tabelBarangBody.innerHTML = ''; semuaDataBarang.forEach(item => { const tr = document.createElement('tr'); tr.innerHTML = `<td>${item.Kode_Barang}</td><td>${item.Nama_Barang}</td><td>${item.Kategori_Barang}</td><td>${item.Stok_Pcs}</td><td>${formatRupiah(item.Harga_Pcs)}</td><td><button class="btn-aksi btn-ubah" data-id="${item.ID_Barang}">Ubah</button><button class="btn-aksi btn-hapus" data-id="${item.ID_Barang}">Hapus</button></td>`; tabelBarangBody.appendChild(tr); }); }
...
// --- MANAJEMEN PENGGUNA (DENGAN PERBAIKAN) ---
...
// --- PERBAIKAN FUNGSI SUBMIT PENGGUNA ---
async function handleFormSubmitPengguna(e) {
    ...
    if (modeEditPengguna) {
        // Saat edit, hanya kirim password jika diisi
        if (passwordValue) {
            dataUntukKirim.append('Password_Baru', passwordValue);
        }
    } else {
        // Saat tambah, password wajib dan namanya adalah 'Password'
        if (!passwordValue) {
...
function masukModeEditPengguna(dataPengguna) { modeEditPengguna = true; formPengguna.scrollIntoView({ behavior: 'smooth' }); for (const key in dataPengguna) { if (formPengguna.elements[key]) { formPengguna.elements[key].value = dataPengguna[key]; } } formPengguna.elements['Username'].value = dataPengguna.Username; formPengguna.elements['Nama_Lengkap'].value = dataPengguna.Nama_Lengkap; formPengguna.elements['Role'].value = dataPengguna.Role; formPengguna.elements['Password'].placeholder = "Isi hanya jika ingin mengubah password"; btnTambahPengguna.classList.add('hidden'); btnSimpanPengguna.classList.remove('hidden'); btnBatalPengguna.classList.remove('hidden'); }
function keluarModeEditPengguna() { modeEditPengguna = false; formPengguna.reset(); idPenggunaInput.value = ''; formPengguna.elements['Password'].placeholder = ""; btnTambahPengguna.classList.remove('hidden'); btnSimpanPengguna.classList.add('hidden'); btnBatalPengguna.classList.add('hidden'); }

// --- LOGIKA KASIR, LAPORAN & STRUK ---
function cariBarang() { const query = inputCari.value.toLowerCase(); if (query.length < 2) { hasilPencarianDiv.classList.add('hidden'); hasilPencarianDiv.innerHTML = ''; return; } const hasilFilter = semuaDataBarang.filter(item => item.Kode_Barang.toLowerCase().includes(query) || item.Nama_Barang.toLowerCase().includes(query)).slice(0, 10); hasilPencarianDiv.innerHTML = ''; if (hasilFilter.length > 0) { hasilPencarianDiv.classList.remove('hidden'); hasilFilter.forEach(item => { const itemDiv = document.createElement('div'); itemDiv.className = 'rekomendasi-item'; itemDiv.innerHTML = `<strong>${item.Nama_Barang}</strong> <br><small>Kode: ${item.Kode_Barang} | Stok: ${item.Stok_Pcs} Pcs</small>`; itemDiv.addEventListener('click', () => pilihBarang(item)); hasilPencarianDiv.appendChild(itemDiv); }); } else { hasilPencarianDiv.classList.add('hidden'); } }
function pilihBarang(item) { formTambahKeranjang.classList.remove('hidden'); itemTerpilihDataInput.value = JSON.stringify(item); namaBarangTerpilihSpan.innerHTML = `${item.Nama_Barang} <small>(Stok: ${item.Stok_Pcs} Pcs)</small>`; selectSatuanKasir.innerHTML = ''; selectSatuanKasir.add(new Option(`Pcs - ${formatRupiah(item.Harga_Pcs)}`, 'Pcs')); if (item.Harga_Lusin > 0 && item.Pcs_Per_Lusin > 0) { selectSatuanKasir.add(new Option(`Lusin - ${formatRupiah(item.Harga_Lusin)}`, 'Lusin')); } if (item.Harga_Karton > 0 && item.Pcs_Per_Karton > 0) { selectSatuanKasir.add(new Option(`Karton - ${formatRupiah(item.Harga_Karton)}`, 'Karton')); } inputCari.value = ''; hasilPencarianDiv.classList.add('hidden'); inputJumlahKasir.value = 1; inputJumlahKasir.focus(); }
function handleTambahKeKeranjang(e) { e.preventDefault(); const itemData = JSON.parse(itemTerpilihDataInput.value); const jumlahDiminta = parseInt(inputJumlahKasir.value); const satuanDiminta = selectSatuanKasir.value; const pcsDiKeranjang = keranjang.filter(item => item.idBarang === itemData.ID_Barang).reduce((total, item) => total + item.jumlahPcs, 0); let pcsAkanDitambah = 0; if (satuanDiminta === 'Pcs') { pcsAkanDitambah = jumlahDiminta; } else if (satuanDiminta === 'Lusin') { pcsAkanDitambah = jumlahDiminta * itemData.Pcs_Per_Lusin; } else if (satuanDiminta === 'Karton') { pcsAkanDitambah = jumlahDiminta * itemData.Pcs_Per_Karton; } if ((pcsDiKeranjang + pcsAkanDitambah) > itemData.Stok_Pcs) { const sisaStokEfektif = itemData.Stok_Pcs - pcsDiKeranjang; alert(`Stok tidak mencukupi!\n\nStok Awal: ${itemData.Stok_Pcs} Pcs\nSudah di Keranjang: ${pcsDiKeranjang} Pcs\nSisa Stok Tersedia: ${sisaStokEfektif} Pcs`); return; } let hargaSatuan = 0; if (satuanDiminta === 'Pcs') { hargaSatuan = itemData.Harga_Pcs; } else if (satuanDiminta === 'Lusin') { hargaSatuan = itemData.Harga_Lusin; } else if (satuanDiminta === 'Karton') { hargaSatuan = itemData.Harga_Karton; } const itemDiKeranjang = { idBarang: itemData.ID_Barang, namaBarang: itemData.Nama_Barang, jumlah: jumlahDiminta, jumlahPcs: pcsAkanDitambah, satuan: satuanDiminta, hargaSatuan: hargaSatuan, subtotal: jumlahDiminta * hargaSatuan, dataAsli: { Harga_Pcs: itemData.Harga_Pcs, Pcs_Per_Lusin: itemData.Pcs_Per_Lusin, Harga_Lusin: itemData.Harga_Lusin, Pcs_Per_Karton: itemData.Pcs_Per_Karton, Harga_Karton: itemData.Harga_Karton } }; const indexAda = keranjang.findIndex(item => item.idBarang === itemDiKeranjang.idBarang && item.satuan === itemDiKeranjang.satuan); if (indexAda > -1) { keranjang[indexAda].jumlah += jumlahDiminta; keranjang[indexAda].jumlahPcs += pcsAkanDitambah; keranjang[indexAda].subtotal = keranjang[indexAda].jumlah * keranjang[indexAda].hargaSatuan; } else { keranjang.push(itemDiKeranjang); } renderKeranjang(); formTambahKeranjang.classList.add('hidden'); inputJumlahKasir.value = 1; inputCari.focus(); }
function renderKeranjang() { tabelKeranjangBody.innerHTML = ''; let total = 0; keranjang.forEach((item, index) => { const tr = document.createElement('tr'); let satuanOptions = `<option value="Pcs">Pcs</option>`; const dataAsli = item.dataAsli; if (dataAsli.Harga_Lusin > 0 && dataAsli.Pcs_Per_Lusin > 0) { satuanOptions += `<option value="Lusin">Lusin</option>`; } if (dataAsli.Harga_Karton > 0 && dataAsli.Pcs_Per_Karton > 0) { satuanOptions += `<option value="Karton">Karton</option>`; } tr.innerHTML = `<td>${item.namaBarang}</td><td><input type="number" class="qty-keranjang" value="${item.jumlah}" min="1" data-index="${index}"></td><td><select class="satuan-keranjang" data-index="${index}">${satuanOptions}</select></td><td>${formatRupiah(item.subtotal)}</td><td><button class="btn-aksi btn-hapus" data-index="${index}">X</button></td>`; tabelKeranjangBody.appendChild(tr); tr.querySelector('.satuan-keranjang').value = item.satuan; total += item.subtotal; }); totalBelanjaSpan.textContent = formatRupiah(total); hitungKembalian(); }
function updateKuantitasKeranjang(index, jumlahBaru) { const item = keranjang[index]; if (!item) return; const itemDataAsli = semuaDataBarang.find(i => i.ID_Barang === item.idBarang); if (!itemDataAsli) { alert('Data barang tidak ditemukan. Coba muat ulang halaman.'); renderKeranjang(); return; } const stokAwal = itemDataAsli.Stok_Pcs; const pcsLainDiKeranjang = keranjang.filter((_, i) => i !== parseInt(index) && keranjang[i].idBarang === item.idBarang).reduce((total, itemLain) => total + itemLain.jumlahPcs, 0); let pcsDimintaSekarang = 0; if (item.satuan === 'Pcs') { pcsDimintaSekarang = jumlahBaru; } else if (item.satuan === 'Lusin') { pcsDimintaSekarang = jumlahBaru * item.dataAsli.Pcs_Per_Lusin; } else if (item.satuan === 'Karton') { pcsDimintaSekarang = jumlahBaru * item.dataAsli.Pcs_Per_Karton; } if ((pcsLainDiKeranjang + pcsDimintaSekarang) > stokAwal) { const sisaStokEfektif = stokAwal - pcsLainDiKeranjang; alert(`Stok tidak mencukupi!\n\nStok Awal: ${stokAwal} Pcs\nItem lain di Keranjang: ${pcsLainDiKeranjang} Pcs\nSisa Stok Tersedia: ${sisaStokEfektif} Pcs`); renderKeranjang(); return; } item.jumlah = jumlahBaru; item.jumlahPcs = pcsDimintaSekarang; item.subtotal = jumlahBaru * item.hargaSatuan; renderKeranjang(); }
function updateSatuanKeranjang(index, satuanBaru) { const item = keranjang[index]; if (!item) return; const itemDataAsliServer = semuaDataBarang.find(i => i.ID_Barang === item.idBarang); if (!itemDataAsliServer) { alert('Data barang tidak ditemukan. Coba muat ulang halaman.'); return; } let hargaSatuanBaru = 0; let jumlahPcsBaru = 0; if (satuanBaru === 'Pcs') { hargaSatuanBaru = itemDataAsliServer.Harga_Pcs; jumlahPcsBaru = item.jumlah; } else if (satuanBaru === 'Lusin') { hargaSatuanBaru = itemDataAsliServer.Harga_Lusin; jumlahPcsBaru = item.jumlah * itemDataAsliServer.Pcs_Per_Lusin; } else if (satuanBaru === 'Karton') { hargaSatuanBaru = itemDataAsliServer.Harga_Karton; jumlahPcsBaru = item.jumlah * itemDataAsliServer.Pcs_Per_Karton; } const stokAwal = itemDataAsliServer.Stok_Pcs; const pcsLainDiKeranjang = keranjang.filter((_, i) => i !== parseInt(index) && keranjang[i].idBarang === item.idBarang).reduce((total, itemLain) => total + itemLain.jumlahPcs, 0); if ((pcsLainDiKeranjang + jumlahPcsBaru) > stokAwal) { alert(`Stok tidak mencukupi untuk mengubah ke satuan ${satuanBaru}!`); renderKeranjang(); return; } item.satuan = satuanBaru; item.hargaSatuan = hargaSatuanBaru; item.jumlahPcs = jumlahPcsBaru; item.subtotal = item.jumlah * hargaSatuanBaru; renderKeranjang(); }
